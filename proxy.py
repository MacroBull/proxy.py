...
# select your favorite mirror here
FORWARD_MAP = {
    ...
    def process(self, data):
        if self.state in (HttpParser.states.HEADERS_COMPLETE,
            ...
            if b'content-length' in self.headers:
                ...
                if self.body_size >= int(self.headers[b'content-length'][1]):
                    self.state = HttpParser.states.COMPLETE
...
class Proxy(threading.Thread):
    ...
    def __init__(self, client, auth_code=None, server_recvbuf_size=65536, client_recvbuf_size=65536):
        ...
        if self.request.state == HttpParser.states.COMPLETE:
            ...
            # and queue for the server with appropriate headers
                ...
                self.server.queue(self.request.build(
                    del_headers=[b'proxy-authorization', b'proxy-connection', b'connection', b'keep-alive', b'host',],
                    add_headers=[(b'Via', b'1.1 proxy.py v%s' % version), (b'Connection', b'Close'), (b'Host', host),]
                ))
...
class HTTP(TCP):
    ...
    def __init__(self, hostname='127.0.0.1', port=8899, backlog=100,
                 auth_code=None, server_recvbuf_size=65536, client_recvbuf_size=65536):
...
def main():
    ...
    parser.add_argument('--server-recvbuf-size', default='65536', help='Default: 64 KB. '
                                                                      ...
                                                                      'increased RAM.')
    parser.add_argument('--client-recvbuf-size', default='65536', help='Default: 64 KB. '
                                                                      'Maximum amount of data received from the '
